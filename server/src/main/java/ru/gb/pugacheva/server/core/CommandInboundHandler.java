...
public class CommandInboundHandler extends SimpleChannelInboundHandler<Command> {
    ...
    ListOfFilesService listOfFilesService; //TODO: временная мера. Скорректировать, чтобы сервис работал через словарь
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Command command) throws Exception {
        if (command.getCommandName().startsWith("login")) {
            ...
            Object[] newArgs = {command.getArgs()[0], absolutePathOfDownloadFile, command.getArgs()[2]};
            ctx.writeAndFlush(new Command("readyToDownload", newArgs));
